...
export const transactionService = {
  ...
  async getDashboardMetrics(): Promise<{
    ...
    const supabase = createClient();
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) return { currentBalance: 0, todayIncome: 0, todayExpense: 0, todayCount: 0, monthIncome: 0, monthExpense: 0 };
    ...
    const totalInitialBalance = accErr || !accounts
      ? 0
      : accounts.reduce((s, a) => s + Number(a.initial_balance), 0);

    // All confirmed transactions (all time) to compute current balance
    const { data: allTxns, error: txErr } = await supabase
      .from('transactions')
      .select('transaction_type, amount, transaction_date, transaction_status')
      .eq('user_id', user.id)
      .eq('transaction_status', 'confirmed');

    if (txErr || !allTxns) {
      return { currentBalance: totalInitialBalance, todayIncome: 0, todayExpense: 0, todayCount: 0, monthIncome: 0, monthExpense: 0 };
    }
    ...
    for (const t of allTxns) {
      ...
      if (t.transaction_date >= firstDay && t.transaction_date <= lastDay) {
        if (t.transaction_type === 'income') monthIncome += amount;
        else monthExpense += amount;
      }
    }

    const currentBalance = totalInitialBalance + confirmedIncome - confirmedExpense;

    return { currentBalance, todayIncome, todayExpense, todayCount, monthIncome, monthExpense };
  },
};
